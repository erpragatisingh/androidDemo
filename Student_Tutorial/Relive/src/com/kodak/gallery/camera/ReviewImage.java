...
public class ReviewImage extends Activity implements View.OnClickListener {
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		int sideButtons = MenuHelper.INCLUDE_DELETE_MENU
				| MenuHelper.INCLUDE_SHARE_MENU | MenuHelper.INCLUDE_SET_MENU
				| MenuHelper.INCLUDE_VIEWPLAY_MENU;

		mImageMenuRunnable = MenuHelper.addImageMenuItems(menu,
				MenuHelper.INCLUDE_ALL & ~sideButtons, ReviewImage.this,
				mHandler, mDeletePhotoRunnable, new MenuHelper.MenuInvoker() {
	...
	public boolean onPrepareOptionsMenu(Menu menu) {
		...
		MenuHelper.enableShowOnMapMenuItem(menu, MenuHelper
				.hasLatLngData(image));
	...
	public boolean onMenuItemSelected(int featureId, MenuItem item) {
		...
		if (mImageMenuRunnable != null) {
			mImageMenuRunnable.aboutToCall(item, mAllImages
					.getImageAt(mCurrentPosition));
		}
		return b;
	}
	...
	void setImage(int pos, boolean showControls) {
		...
		if (b != null) {
			...
			mImageView.setImageRotateBitmapResetBase(new RotateBitmap(b, image
					.getDegreesRotated()), true);
			updateZoomButtonsEnabled();
		}
	...
	private boolean init(Uri uri) {
		if (uri == null)
			return false;
		mAllImages = (mParam == null) ? buildImageListFromUri(uri)
	...
	public void onSaveInstanceState(Bundle b) {
		...
		b.putParcelable(STATE_URI, mAllImages.getImageAt(mCurrentPosition)
				.fullSizeImageUri());
	...
	private void startShareMediaActivity(IImage image) {
		...
		try {
			startActivity(Intent.createChooser(intent,
					getText(isVideo ? R.string.sendVideo : R.string.sendImage)));
		} catch (android.content.ActivityNotFoundException ex) {
			Toast.makeText(
					this,
					isVideo ? R.string.no_way_to_share_image
	...
	public void onClick(View v) {
		...
		case R.id.btn_delete:
			MenuHelper.deleteImage(this, mDeletePhotoRunnable, mAllImages
					.getImageAt(mCurrentPosition));
			break;
		case R.id.btn_play:
		...
		try {
			...
			case KeyEvent.KEYCODE_DPAD_CENTER: {
				if (mViewImage.isPickIntent()) {
					...
					mViewImage.setResult(ReviewImage.RESULT_OK, new Intent()
			...
			case KeyEvent.KEYCODE_DEL:
				MenuHelper.deletePhoto(mViewImage,
						mViewImage.mDeletePhotoRunnable);
				break;
			}
		} finally {
			if (nextImagePos >= 0
